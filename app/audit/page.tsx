...
export default function AuditPage() {
  ...
  const guardrailCount = logs.filter(
    (log) =>
      log.action.startsWith("GUARDRAIL_")
  ).length;

  const agentCount = logs.filter(
    (log) =>
      log.actor?.includes("REVIVEAI")
  ).length;
...
function getEventType(action: string) {
